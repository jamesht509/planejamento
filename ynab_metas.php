...
<div class="container-xl px-3 pb-5">
  <h4 class="mb-4 text-center">🎯 Metas por Categoria</h4>
    ...
    <form action="" method="POST" class="row g-3">
      <div class="col-md-6">
        <select name="categoria_id" class="form-select" required>
          ...
          <?php
          $cats = $pdo->query("SELECT * FROM categorias WHERE tipo = 'despesa'");
          foreach ($cats as $cat) {
      ...
      <div class="col-md-4">
        <input type="number" step="0.01" name="valor_meta" class="form-control" placeholder="Valor da Meta" required>
      </div>
      <div class="col-md-2">
    ...
    <table class="table table-bordered table-hover align-middle">
      ...
      <tbody>
        ...
        if (isset($_POST['salvar'])) {
          $stmt = $pdo->prepare("INSERT INTO metas (categoria_id, valor_meta) VALUES (?, ?)");
          $stmt->execute([$_POST['categoria_id'], $_POST['valor_meta']]);
          header("Location: ynab_metas.php");
          exit;
        }
        ...
        $sql = "SELECT m.id, c.nome as categoria, m.valor_meta
                FROM metas m JOIN categorias c ON m.categoria_id = c.id
                ORDER BY c.nome";
        $stmt = $pdo->query($sql);
        foreach ($stmt as $meta) {
          echo "<tr>
                  ...
                  <td><a href='?excluir={$meta['id']}' onclick='return confirm(\"Excluir meta?\")' class='btn btn-sm btn-danger'>🗑️</a></td>
                </tr>";
        }
        ?>
      </tbody>
