...
<head>
  ...
  <style>
    ...
    .theme-toggle {
      font-size: 18px; background: transparent; border: none; margin-left: 12px; color: inherit; cursor: pointer;
    }
    .fixed-nav {
    ...
    .fixed-nav a {
      flex: 1; text-align: center; text-decoration: none; color: #555; font-size: 13px;
    }
    .fixed-nav a div {
      display: flex; flex-direction: column; align-items: center; font-weight: 500;
    }
    .fixed-nav a span {
      ...
      <tbody>
        ...
        foreach ($stmt as $row) {
          ...
          echo "<tr>
                  ...
                  <td>{$row['descricao']}</td>
                  <td>
                    <a href='editar_transacao.php?id={$row['id']}' class='btn btn-sm btn-warning'>✏️</a>
                    <a href='excluir_transacao.php?id={$row['id']}' onclick='return confirm(\"Confirma?\")' class='btn btn-sm btn-danger'>🗑️</a>
                  </td>
                </tr>";
        }
        ?>
...
<div class="fixed-nav">
  <a href="dashboard_ynab.php"><div>📊<span>Dashboard</span></div></a>
  <a href="nova_transacao.php"><div>➕<span>Nova</span></div></a>
  <a href="listar_transacoes.php"><div>📄<span>Transações</span></div></a>
  <a href="categorias.php"><div>📂<span>Categorias</span></div></a>
  <a href="contas.php"><div>🏦<span>Contas</span></div></a>
</div>
